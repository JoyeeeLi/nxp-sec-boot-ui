...
do
    ...
    openssl ca -batch -passin file:./key_pass.txt \
               ...
               -out ../crts/SRK${i}_sha256_${kl}_65537_v3_ca_x509_crt.pem \
               -days ${val_period} \
               -config ../ca/openssl.cnf

    # Convert SRK Certificate to DER format
    openssl x509 -inform PEM -outform DER \
                 -in ../crts/SRK${i}_sha256_${kl}_65537_v3_ca_x509_crt.pem \
                 -out ../crts/SRK${i}_sha256_${kl}_65537_v3_ca_x509_crt.der
    ...
    openssl ca -batch -md sha256 -outdir ./ \
               ...
               -in ./temp_csf_req.pem \
               -cert ../crts/SRK${i}_sha256_${kl}_65537_v3_ca_x509_crt.pem \
               -keyfile ./SRK${i}_sha256_${kl}_65537_v3_ca_key.pem \
               -extfile ../ca/v3_ca.cnf \
               -out ../crts/CSF${i}_1_sha256_${kl}_65537_v3_ca_x509_crt.pem \
               -days ${val_period} \
               -config ../ca/openssl.cnf

    # Convert CSF Certificate to DER format
    openssl x509 -inform PEM -outform DER \
                 -in ../crts/CSF${i}_1_sha256_${kl}_65537_v3_ca_x509_crt.pem \
                 -out ../crts/CSF${i}_1_sha256_${kl}_65537_v3_ca_x509_crt.der
    ...
    openssl ca -batch -md sha256 -outdir ./ \
               -passin file:./key_pass.txt \
               -in ./temp_img_req.pem \
               -cert ../crts/CSF${i}_1_sha256_${kl}_65537_v3_ca_x509_crt.pem \
               -keyfile ./CSF${i}_1_sha256_${kl}_65537_v3_ca_key.pem \
               -extfile ../ca/v3_usr.cnf \
               -out ../crts/IMG${i}_1_sha256_${kl}_65537_v3_usr_x509_crt.pem \
               -days ${val_period} \
               -config ../ca/openssl.cnf

    # Convert IMG Certificate to DER format
    openssl x509 -inform PEM -outform DER \
                 -in ../crts/IMG${i}_1_sha256_${kl}_65537_v3_usr_x509_crt.pem \
                 -out ../crts/IMG${i}_1_sha256_${kl}_65537_v3_usr_x509_crt.der
...
do
    # Convert SRK certs
    ../linux/x5092wtls \
        -c ../crts/SRK${i}_sha256_${kl}_65537_v3_ca_x509_crt.pem \
        -k ${ca_key}.pem \
        -w ../crts/SRK${i}_sha256_${kl}_65537_v3_ca_crt.der \
        -p ./key_pass.txt

    # Convert CSF certs
    ../linux/x5092wtls \
         -c ../crts/CSF${i}_1_sha256_${kl}_65537_v3_ca_x509_crt.pem \
         -k ./SRK${i}_sha256_${kl}_65537_v3_ca_key.pem \
         -w ../crts/CSF${i}_1_sha256_${kl}_65537_v3_ca_crt.der \
         -p ./key_pass.txt

    # Convert IMG certs
    ../linux/x5092wtls \
         -c ../crts/IMG${i}_1_sha256_${kl}_65537_v3_usr_x509_crt.pem \
         -k ./CSF${i}_1_sha256_${kl}_65537_v3_ca_key.pem \
         -w ../crts/IMG${i}_1_sha256_${kl}_65537_v3_usr_crt.der \
         -p ./key_pass.txt
    i=$((i+1))
done
...
while [ $i -le $num_srk ]
do
    printf "../crts/SRK${i}_sha256_${kl}_65537_v3_ca_crt.der," >> srk_list.txt
    i=$((i+1))
done
read srk_list < srk_list.txt
\rm srk_list.txt
../linux/srktool --hab_ver 3 --certs ${srk_list} \
